...
function trae_dato($odb,$tablap,$valor){
...
function lista($odb,$tabla,$id_basico,$dtbasic){
  ...
  for($i=0;$i<$tabla->numCampos;$i++){
    if( $tabla->Key[$i] == "MUL" ){
      $otabspri[$i] = new Tabla($odb,$tabla->tab_pri[$i]);
      $a_tabpri[$i] = $otabspri[$i]->lee("");
    }    
  }
  ...
  for($x=0; $x<count($r);$x++){
    echo "<tr>";
    foreach($r[$x] as $key=>$valor){
      if( $key == 'password' || $key == 'paswd'){ $valor = '*****';} 

      if($basicos->existe($odb,$tabla,$key)){
        $valor = $basicos->getDtBasico($odb,$valor);
      } else
      if(strpos($key,"estado") !== false ){
...
<body  >        
  ...
  for($i=0;$i<$tabla->numCampos;$i++){
    if(strpos($tabla->Cam[$i],"estado") !== false ){
      $tabla->sel_dep[$i] = "S";
    } 
...
$salida .= "
  <div class='btn-group'>
    <button type='submit' name='btsav' class='btn btn-primary btn-sm' id='btsav' ".$disa." >Guardar</button>
  ...
  <input type='hidden' name='op' value='".$op."' size='1'>
</form> 
</div>
