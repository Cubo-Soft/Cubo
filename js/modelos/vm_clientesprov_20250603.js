...
/*function crearVMClientesProv(data, opcion) {
    if (opcion === 1) {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                var obj = JSON.parse(respuesta);
                $("#divBodyClientesProvisionales").removeClass("bg-light-blue disabled color-palette");
                if (obj.length > 0) {
                    $("#mensajesClientesProvisionales").html('<div class="callout callout-success">Cliente provisional creado de manera exitosa. Desea continuar con la toma del requerimiento?</div>');
                } else {

                    $("#mensajesClientesProvisionales").html('<div class="callout callout-danger">Esto es verdaderamente vergonzoso!. Ha fallado la creación del cliente provisional. Por favor presione la combinación de teclas "CTRL"+ "SHIFT" +"R" e inicie nuevamente. Si vuelve a fallar la creación por favor informe! </div>');
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
