...
function lee_trm($t,$dias,$id_mon=35){ // lee ultimos trm 
    $a = $t->lee(" WHERE id_moneda=$id_mon AND trm <> '0' ORDER BY fecha DESC LIMIT ".$dias,0,"A");
    $son = count($a);
    if($son <1){
        return false;
    }
...
function act_asesor($idreq=0){
	...
	if( $idreq < 1 ){
        ...
        $tb_clipro = new Tabla($odb,"vm_clientesprov");
        $arf = fopen("salida_asesor","w+");
        ...
        if( $clien_provis ){
            $ar_clienprov = $tb_clipro->leec("id_region"," WHERE nit_cliente = '".$ar_req[0]['nit_cliente']."'",0,"A");
            $id_region = $ar_clienprov[0]['id_region'];
        }else{
            $ar_suc = $tb_suc->leec("id_region"," WHERE id_sucursal = $id_suc",0,"A");
            $id_region = $ar_suc[0]['id_region'];
        }
        $ar_reg = $tb_region->lee(" WHERE id_region='".$id_region."'",0,"A");
        if(empty($ar_reg)){
...
function fpesos($vr){
    return number_format($vr,2,',','.'); // formato latino, coma (,) separa decimales, miles con punto (.) 1.234.567,00
}
