...
function trae($ar_aislamtb,$aislam,$diametro){
  $ar = array();
  for($va=0;$va<$ar_aislamtb;$va++){
    if( $ar_aislamtb[$va]['id_aislam'] == $aislam &&
        $ar_aislamtb[$va]['id_diametro'] == $diametro  
      ){
        $ar['valor'] = $ar_aislamtb[$va]['valor'];
        $ar['id']    = $ar_aislamtb[$va]['id_vraislam']; 
        return $ar;
    }
  }
}
...
$ar_aislamtb = $tabla->lee(" ORDER BY id_vraislam,id_diametro,id_aislam");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<form name='Interface' method='post' onkeypress='return anular(event)' enctype='multipart/form-data'>
      ...
      <table class="table table-striped table-hover table-bordered table-sm table-responsive-sm caption-top tabla1">
        <tbody> 
            ...
            <tr>
              ...
              <th colspan='<?php echo count($ar_aislam) -1 ?>' style='text-align:center;font-size:14px;background-color:#BEA537'> 
                   Aislamiento
              </th>
            </tr>
            <tr>
              ...
              <?php
                  for($a=1;$a<count($ar_aislam);$a++){
                    echo "<th style='color:white;text-align:center;background-color:#BEA537;'>".$ar_aislam[$a]['valor']."</th>";
                  }
              //}  
              ?>
            </tr>   
            ...
            <?php
            for($d=0;$d<count($ar_diametros);$d++){
                echo "<tr><td align='center'><B>".$ar_diametros[$d]['nom_diametro']."</B></td>";
                for($a=1;$a<count($ar_aislam);$a++){
                    //                           id= _ele_com_dia
                    $vr_id = "_".$ar_diametros[$d]['id_diametro']."_".$ar_aislam[$a]['id_aislam']; 
                    $ar = trae($ar_aislamtb,$ar_aislam[$a]['id_aislam'],$ar_diametros[$d]['id_diametro']); 
                    $valor = $ar['valor'];
                    $vr_id .= "_".$ar['id'];
  ...
  <script src="../js/vistas/aislamiento_tb.js"></script>  
</body>
</html>