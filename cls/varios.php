...
function lee_trm($t, $dias, $id_mon = 35)
{ // lee ultimos trm 
...
function lee_politrm($moneda = "USD")
{ // lee politica de trm
    ...
    switch ($a_po[0]['manejo']) {
        case "porcentaje":
            $trm = ($trm_act * (1 + ($a_po[0]['valor'] / 100)));
            $opcion = "porctje";
            break;
        case "dias":
            $trm = lee_trm($cmtrm, (int) $a_po[0]['valor'], $id);
            $opcion = "dias";
            break;
        default:
            $trm = $trm_act;
...
function act_asesor($idreq = 0)
{
    ...
    if ($idreq < 1) {
        ...
        $cmps = "codemple,id_estado";
        $w = " WHERE id_cargo IN ( SELECT idcargo FROM vp_jefe_grupo WHERE codgrupo='$codgrupo') ";
        ...
        //ajuste para cambios de cliente provisional Daniel2025
        $ar_req = $tb_req->leec("suc_cliente,grupo,de_linea,cod_trans", " WHERE id_requerim='$idreq'", 0, "A");

        if (empty($ar_req)) {
            exit('No se encontraron resultados para el id_requerim: ' . $idreq);  // Maneja el error como necesites
        }
