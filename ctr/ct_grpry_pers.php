            ...
            if(!isset($_REQUEST['id_prycosto'])){
                ...
                if(!is_array($arreglo)){
                    ...
                    for( $x=0; $x < count($arreglo); $x++ ){
                        $reg  = " id_prycosto:".$id_prycosto." concepto: ".$arreglo[$x]['concepto']." cant: ".$arreglo[$x]['cant'];
                        $reg .= " personas: ".$arreglo[$x]['personas'];
                        ...
                        if( $arreglo[$x]['personas'] == "" || $arreglo[$x]['personas'] == null || is_nan($arreglo[$x]['personas'])){
                            $arreglo[$x]['personas'] = 0;
                        } 
                        $ar = [ 'id_prydet'=>null,'id_prycosto'=>$id_prycosto,
                                'orden'=>($x+1),'concepto'=>$arreglo[$x]['concepto'],
                                'cant'=> $arreglo[$x]['cant'], 'personas' => $arreglo[$x]['personas'],
                                'valor_un'=>$arreglo[$x]['valor_un'],'valor_total'=>$arreglo[$x]['valor_total'],'fechora_act'=>date('Y-m-d H:i:s'),
                                'grabador'=>$_SESSION['codusr'] ];
                                //fwrite($arf,"QUEDA REGISTRO:".json_encode($ar)."\n");
                        $w = " WHERE id_prycosto=".$id_prycosto." AND concepto='".$arreglo[$x]['concepto']."'";        
                        $ar_ex = $tprypers->lee($w,0,"A");
                        $salta = false;
                        if(!empty($ar_ex) ){
                            //fwrite($arf,"El registro EXISTE !\n");
                        ...
                        if(!$salta){
                            //fwrite($arf,"El registro NO existe !\n");
                            $r = $tprypers->ins($ar);
                            if(!$r){
                                //fwrite($arf,"error: ".$r);
                                $ar_sale['sale'] = $r;
