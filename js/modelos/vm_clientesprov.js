        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                if (obj.length > 0) {
                    ...
                    $("#ap_regiones2").val(obj[0]['id_region']).trigger("change");
...
function crearVMClientesProv(data, opcion) {
    if (opcion === 1) {
        ...
        if ($("#nombre_cp").val() === "" || $("#nit_cliente_cp").val() === "" || $("#telefono_cp").val() === "" || $("#email_cp").val() === ""  || $("#region_cp").val() === "") {
            // Si algún campo obligatorio está vacío, mostrar un mensaje de advertencia
            $("#mensajesClientesProvisionales").html('<div class="callout callout-warning">Por favor, complete todos los campos obligatorios (Nombre, NIT, Teléfono, Correo Electrónico, Region).</div>');
...
$(document).on("click", "#modificarClienteProvisional", function () {
    if ($("#nombre_cp").val() === "" || $("#nit_cliente_cp").val() === "" || $("#telefono_cp").val() === "" || $("#email_cp").val() === "" || $("#ap_regiones2").val() === "") {
        $("#mensajesClientesProvisionales").html('<div class="callout callout-warning">Por favor, complete todos los campos obligatorios (Nombre, NIT, Teléfono, Correo Electrónico, Region).</div>');
        return;
    }
    ...
    console.log("ID Región seleccionada:", $("#ap_regiones2").val());
