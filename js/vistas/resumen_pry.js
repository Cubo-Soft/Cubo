...
function multi(obj_can,vu,tot,grtot,opc){
    let nombre = tot+"_x";
    let tot1 = tot;
    let id_can = $(obj_can).attr('id');
    let campo_per = "per" + id_can.slice(3);
    $("#"+campo_per).val( da_valor( $("#"+campo_per) ) );
    //console.log("valor de "+campo_per+": "+da_valor( $("#"+campo_per)) );
    let valor = ( da_valor($(obj_can)) * da_valor($("#"+vu)) * da_valor($("#"+campo_per)) );
...
function autPersonas(obj_can){ // automatiza el valor default para el nuevo campo personas.
    let id_can = $(obj_can).attr('id');
    let valor_can = da_valor( $(obj_can) );
    let campo_per = "per" + id_can.slice(3);
    let valor_per = da_valor( $("#"+campo_per) );
    if( !isNaN(valor_can) && valor_can > 0 ){
        //console.log("valor_can ES NUMERO Y > 0 : "+valor_can+" campo_per: "+campo_per);
        if( valor_per == null || valor_per < 1 ){
            $("#"+campo_per).val(1);
        }
    }else{
        $(obj_can).val( da_valor( $(obj_can) ) );
    }    
}
...
function carga_insValv(){
    ...
    arg_InsValv[ult]["diamInsValv"] = parseFloat( $("#diamInsValv_0 option:selected").text() );
    arg_InsValv[ult]["cantInsValv"] = parseFloat( $("#cantInsValv_0").val() ); 
    arg_InsValv[ult]["esAiInsValv"] = parseFloat( $("#esAiInsValv_0 option:selected").text() );
...
function cst1(obj,id_08060){
    ...
    //console.log("CS1:grupo: "+grupo+" linea: "+linea);
...
function act_cst_tuberia(){  // Actualiza costos Instalacion Tuberia
    ...
    if( Array.isArray(arg_sopInsTub) ){
        ...
        let destino = "../ctr/ct_grpry_soporteria.php";
        let asin = false;
        console.log("ENVIANDO en tuberia: "+JSON.stringify($daton));
        procesa($daton,destino,asin); 
    }
    ...
    if( Array.isArray(arg_InsValv) ){
        ...
        for(let x=0; x < arg_InsValv.length;x++){
            dato[x] = { 
                ...
                'diam':arg_InsValv[x]["diamInsValv"],
                'cantidad':arg_InsValv[x]["cantInsValv"],
                'aislam':arg_InsValv[x]["esAiInsValv"],
...
function carga_arreglo_valvulas( arr ){
    for(let x=0; x < arr.length;x++){
        ...
        arg_InsValv[x]["diamInsValv"]     = arr[x].diam;
        arg_InsValv[x]["cantInsValv"]     = arr[x].cantidad;  
        arg_InsValv[x]["esAiInsValv"]     = arr[x].aislam; 
...
function act_cst_varios(){
    ...
    console.log("ENVIANDO en varios: "+JSON.stringify($dato));
