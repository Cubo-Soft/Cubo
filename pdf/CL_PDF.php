...
class CL_PDF extends TCPDF
{
    ...
    function mostrarTablaRepuestos($datos, $option)
    {
        ...
        $totalDescuentos = 0;
        $totalesIva = 0;
        $maxSemanaEntrega = 0; // ← esta es nueva Diego 26-06-2025
        ...
        */
        ...
        // Mostrar observaciones generales si existen (al final de la tabla)
        $observacionesFinal = '';
        foreach ($datos["vr_cotizadet"] as $item) {
            if (!empty($item["observs"])) {
                $observacionesFinal = $item["observs"];
                break; // toma solo la primera observación general
            }
        }

        if (!empty($observacionesFinal)) {
            $this->html .= '<tr><td colspan="11" border="1" style="font-size: 7px; text-align: left;"><b>Observaciones:</b> ' . $observacionesFinal . '</td></tr>';
        }
        //Ajsute visual del documento para organizar información Diego 27-06-2025
        $this->html .= "</table>";
        $direccionEntrega = $datos["nm_sucursal"][0]["nom_pais"] . ',' . $datos["nm_sucursal"][0]["nom_ciudad"] . ',' . $datos["nm_sucursal"][0]["direccion"];

        // Espacio visual antes del bloque inferior
        $this->html .= '<br>';

        $this->html .= '<span style="text-align:center; font-size: 9px;"><b>CONDICIONES COMERCIALES</b></span><br>';

        $this->html .= '<span style="font-size: 7px;"><b>FORMA DE PAGO:</b> ' . $datos["vr_cotiza"][0]["terminoPago"] . '</span><br>';
        $this->html .= '<span style="font-size: 7px;"><b>MONEDA:</b> ' . $datos["vr_cotiza"][0]["moneda"] . '</span><br>';
        $this->html .= '<span style="font-size: 7px;"><b>ASESOR COMERCIAL:</b> ' . $datos["nm_empleados"][0]["nombreEmpleado"] . '</span><br>';
        $this->html .= '<span style="font-size: 7px;"><b>TELÉFONO:</b> ' . $datos["nm_empleados"][0]["telefono"] . '</span><br>';

        $direccionEntrega = $datos["nm_sucursal"][0]["nom_pais"] . ', ' . $datos["nm_sucursal"][0]["nom_ciudad"] . ', ' . $datos["nm_sucursal"][0]["direccion"];
        $this->html .= '<span style="font-size: 7px;"><b>LUGAR DE ENTREGA:</b> ' . $direccionEntrega . '</span><br>';

        $this->html .= '<span style="font-size: 7px;"><b>VALIDEZ DE LA OFERTA:</b> ' . $datos["vp_vigencia"][0]["descrip"] . ' A PARTIR DE LA FECHA</span><br>';

        // Línea comentada en caso de reactivación posterior Diego 27-06-2025
        // $this->html .= '<span style="font-size: 7px;"><b>TIEMPO DE ENTREGA:</b> ' . ($maxSemanaEntrega === 0 ? 'INMEDIATA' : $maxSemanaEntrega . ' SEMANA(S)') . '</span><br>';


        // 🔇 Desactiva errores que imprimen texto antes del PDF (para evitar "Some data has already been output") Diego 27-06-2025
