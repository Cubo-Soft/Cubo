        ...
        for (var i = 0; i < data["vr_cotizadet"].length; i++) {
            ...
            if (data["caracteristicasRepuestos"][i].length > 0) {
                ...
                if (cod_items.indexOf(data["vr_cotizadet"][i]["id_orden"]) === -1) {
                    ...
                    valorIva = (valorConDescuento * parseFloat(datos.iva)) / 100;
...
function crearTablaSucursales(data, opcion) {
    ...
    tabla += '<thead>';
    tabla += '<tr class="bg-primary">';
    tabla += '<th>No.</th><th>Nombre</th><th>Dirección</th><th>Teléfono</th><th>Ciudad</th><th>País</th><th>Región</th><th>Estado</th><th>Mod</th><th>Conctactos</th>';
    tabla += '</tr>';
    tabla += '</thead>';
    tabla += '<tbody>';
