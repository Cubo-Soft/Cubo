            ...
            success: function (respuesta) {
                ...
                if (obj["vr_requerimdet"].length === 0) {
                    ...
                    for (var i = 0; i < obj["vr_requerimdet"].length; i++) {
                        ...
                        //para los equipos
                        if (obj["vr_requerimdet"][i]["misional"] === '01' || obj["vr_requerimdet"][i]["misional"] === '04') {
                        ...
                        if (obj["vr_requerimdet"][i]["misional"] === '03') {
                            ...
                            $("#divServicioMantenimiento").html(armarTablaMantenimientos(obj, 2));
                        }
                    }

                    //para los equipos
                    if (obj["vr_requerimdet"][0]["misional"] === '01' || obj["vr_requerimdet"][0]["misional"] === '04') {
                    ...
                    if (obj["vr_requerim"][0]["estado"] === 83 || obj["vr_requerim"][0]["estado"] === 84) {
                        $("#adicionarSucursal").hide();
                    } else {
                        $("#iniciarCotizacion").attr('name', obj["vr_requerim"][0]["id_requerim"]);
                        $("#divBotonRecargar").show();
                        $("#botonLimpiarAbajo").hide();
                    }

                    if (obj["vr_requerim"][0]["estado"] === 82 || obj["vr_requerim"][0]["estado"] === 84) {
                    ...
                    if (obj["vr_requerim"][0]["estado"] === 83) {
                        $("#divLineasMisionales").html(obj["vr_requerimdet"][0]["nom_grupo"]);
                ...
                alert("Error en function retornarVRRequerim(data, opcion=4) {...\nError desde el servidor. Por favor informe a soporte");
            }
        });
    }
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function (respuesta) {
                        var obj = JSON.parse(respuesta);
                        if (obj) {
                            $("#divMensajesRequerimientos").html('<div class="callout callout-success">Asesor asignado de manera correcta</div>');
                        }
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.log("Error en la solicitud AJAX:");
